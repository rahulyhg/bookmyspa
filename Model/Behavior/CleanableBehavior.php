...
class CleanableBehavior extends ModelBehavior{
	...
	public function doCleanValue($value, $options=array()) {
		...
		if ($options['escape']) {
			//preventing from SQL Injection-SQL Injection ---- http://www.symantec.com/connect/articles/detection-sql-injection-and-cross-site-scripting-attacks
			//$value = Sanitize::escape($value);
			//$value = preg_replace("/[(\%27)|(\')|(\-\-)|(\%23)|(#)|(*)]/ix", "", $value); //Regex for detection of SQL meta-characters like -- , ', #,*
			$value = preg_replace('/exec(\s|\+)+(s|x)p\w+/ix', '', $value); // Regex for detecting SQL Injection attacks on a MS SQL Server
			$value = preg_replace('/((\%3D)|(=))[^\n]*((\%27)|(\')|(\-\-)|(#)|(\%3B)|(;))/i', '', $value); // Modified regex for detection of SQL meta-characters
			$value = preg_replace('/\w*((\%27)|(\'))((\%6F)|o|(\%4F))((\%72)|r|(\%52))/ix', '', $value); // Regex for typical SQL Injection attack
			//$value = preg_replace('/((\%27)|(\'))union/ix', '', $value); // Regex for detecting SQL Injection with the UNION keyword
			$value = preg_replace('/union/ix', '', $value); // Regex for detecting SQL Injection with the UNION keyword
			$value = preg_replace('/select/ix', '', $value); // Regex for detecting SQL Injection with the SELECT keyword
			$value = preg_replace('/insert/ix', '', $value); // Regex for detecting SQL Injection with the INSERT keyword
			$value = preg_replace('/update/ix', '', $value); // Regex for detecting SQL Injection with the UPDATE keyword
			$value = preg_replace('/delete/ix', '', $value); // Regex for detecting SQL Injection with the DELETE keyword
			$value = preg_replace('/drop table/ix', '', $value); // Regex for detecting SQL Injection with the DROP keyword
		}
		return $value;
	}
	/**
