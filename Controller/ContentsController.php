...
App::import('Vendor', 'xml_regex');
class ContentsController extends AppController {
    var $name = 'Contents';
    public $components = array('Paginator', 'Common','Cookie');
    ...
    function home(){
        ...
        $sevendaystimeststamp = StrToTime('-7 days', $currenttimeStamp);
        $currentd = date('Y-m-d H:i:s', $sevendaystimeststamp);
		
        $popular_posts = $this->Content->find('all', array('conditions' => array('Content.created >'=>$currentd,'Content.status' => 1),'order'=>array('Content.views DESC'),'limit' => 5));
        ...
        foreach($contents as $content) { //pr($content);
	     $postid = $content['Content']['id'];
            if (!empty($content['Content']['title'])) {
                $title = substr($content['Content']['title'], 0, 70);
            } else {
                $title ='';
            }
            ...
            if (!empty($user['User']['avatar_image'])) {
                if (filter_var($user['User']['avatar_image'], FILTER_VALIDATE_URL) === FALSE) {
                    $primage = '<a href=/users/profile_post/'.$user['User']['id'].'><img src="/img/avatar/' . $user['User']['avatar_image'].'" style="max-width:10%" /></a>';   
                   }else{
                    $primage = '<a href=/users/profile_post/'.$user['User']['id'].'><img src="' . $user['User']['avatar_image'] . '" style="max-width:10%" /></a>';  
                   }
                   $primage = '<a href=/users/profile_post/'.$user['User']['id'].'><img src="' . $user['User']['avatar_image'] . '" style="max-width:10%" /></a>';
                    } else {
                   $primage = '<a href=/users/profile_post/'.$user['User']['id'].'><img src="/img/avatar/no_image.png" style="max-width:10%" /></a>';
            }
              
            if (!empty($content['Photo'])) {
                if ($content['Photo'][0]['image_type'] == 'file') {
                    $photoimage = '<a href=/post/'.$postid.'><img src="/timthumb.php?src=/img/photos/thumb/'.$content['Photo'][0]['image'].'&h=191&w=236&zc=1"></a>';
                } else {
                    $photoimage = '<a href=/post/'.$postid.'><img src="'.$content['Photo'][0]['image'].'"></a>';
                }
            } else {
                 App::import('Helper', 'Common');
                 $ParserHelper->Common = new CommonHelper(new View());   
                 $video = $ParserHelper->Common->get_videothumb($postid); 
                       if($video){
                        $photoimage =   '<img src='.$video.' />'; 
                        }else{
                        $photoimage =  '<img src="/timthumb.php?src=/img/photos/thumb/soccer-ball-128.png&h=191&w=236&zc=1">';
                        }
                        $photoimage = '<a href=/post/'.$postid.'>'.$photoimage.'</a>';
            }
            ...
            echo '<section class="col-sm-4 product">
                ...
                <h4><a href=/post/'.$postid.'>'.$title.'</a></h4>
                <section class="user-sec">
                    <span class="userpic">'.$primage.'</span>
                    <a href=/users/profile_post/'.$user['User']['id'].'><span class="post-time"><strong>' . $firstname . '</strong></a><span style="color:#a6a6a6;">' . $dayss . '</span></span>
                </section>
            </section>';
        }
        exit;
	 ...
	 function post_details($Pid=NULL){
		...
		$this->loadModel('PollPreviewResult');
		 ...
		 $page_view['IP_address']=$_SERVER['REMOTE_ADDR']; 
                 ...
                 if(@$_GET['uid']){
                    $this->loadModel('User');   
                    $user_revenue_arr = $this->User->find('first', array('recursive' => -1, 'fields' => array('revenue_percent'), 'conditions' => array('User.id' => base64_decode($_GET['uid']))));   
                    
                    $profit = '.5'; /* 50 % for sub owner  */
                    $page_view['uid']=  base64_decode($_GET['uid']);
                    $page_view['page_count']=$page_count;
                    $page_view['profit']=$profit;
                    $page_view['rate']= isset($user_revenue_arr['User']['revenue_percent']) ? $user_revenue_arr['User']['revenue_percent'] : 2;
                    $this->PageView->create();
                    $this->PageView->save($page_view);
                    $page_count = 0;
                 } 
                $user_id = $this->Content->find('first' , array('conditions'=>array('Content.id'=>$Pid), 'contain' => array('User'), 'fields'=>array('User.revenue_percent', 'Content.user_id')));
                
                $page_view['page_count']=$page_count;
                $page_view['profit']=$profit;
		$page_view['uid']=$user_id['Content']['user_id'];
                $page_view['rate']= isset($user_id['User']['revenue_percent']) ? $user_id['User']['revenue_percent'] : 2;
		$this->PageView->create();
		$this->PageView->save($page_view);
		$this->Content->id=$Pid;
		...
		$contentfirst = $this->Content->find('first',array('conditions'=>array('Content.id'=>$Pid,'Content.content_order !='=>'null'),'fields'=>array('Content.content_order'))); 
		$indexvalue = array();
		...
		$checkguestdata=$this->PollPreviewResult->find('first',array('conditions'=>array('PollPreviewResult.IP_address'=>$_SERVER['REMOTE_ADDR'])));
		$this->set(compact('content','users','allpostcomments','allpostcounts','tagIDs','count_page_view','lastcommentid','indexvalue','checkguestdata','CCC'));
	}
			...
			foreach($allpostcomments as $allpostcomment){
				...
				if (!empty($allpostcomment['PostComment']['comment_image']) && ($allpostcomment['PostComment']['image_type']=='File')){
					$imagetype = '<img src="'.$allpostcomment['PostComment']['comment_image'].'"
								  onmouseover=this.src="'.$allpostcomment['PostComment']['comment_image'].'"
								 onmouseout=this.src="'.$allpostcomment['PostComment']['comment_image'].'">
							</img>';
				}else if (!empty($allpostcomment['PostComment']['comment_image']) && ($allpostcomment['PostComment']['image_type']=='Gallery')){
					 $filename = basename($allpostcomment['PostComment']['comment_image']);  
					$imagetype = '<img src="'.$allpostcomment['PostComment']['comment_image'].'"
								 onmouseover=this.src="/img/comments/original/'.$filename.'"
								 onmouseout=this.src="'.$allpostcomment['PostComment']['comment_image'].'">
							</img>';
				...
				$username = $user['User']['username'];
                                 ...
                                 if (!empty($user['User']['avatar_image'])){
                                   $img = $user['User']['avatar_image'];
                                    if (filter_var($img, FILTER_VALIDATE_URL) === FALSE) {
                                      $primage =   '<a href="/users/profile_post/'.$user['User']['id'].'"><img src="/img/avatar/'.$img.'" style="max-width:10%" /></a>';
                                       }else{
                                           $primage = '<a href="/users/profile_post/'.$user['User']['id'].'"><img src='.$img.' style="max-width:10%" /></a>';
                                       }   
                                        }else {
                                    $primage = '<a href="/users/profile_post/'.$user['User']['id'].'"><img src="/img/avatar/no_image.png" style="max-width:10%" /></a>';
                                  }
					   ...
					   if(empty($allpostcomment['PostComment']['parent_id'])){
										...
										<span class="userpic">'.$primage.'</span>
											<span class="post-time"><a href="/users/profile_post/'.$user['User']['id'].'">'.$username.'</strong></a></span>
										</section>
									...
									<section class="user-comment-txt" >'.$imagetype.'<p>'.$postcommnt.'</p>
										...
										<section class="user-reply">
											<span class="reply-text">
												<a href="javascript:void(0)" class="replace_tec" data-tec="reply"  onclick="addcommentbox('.$postid.','.$parentids.','.$parentids.','.$lastcommentid.')">Reply</a>
											</span>
									  ...
									  if($num_day == '0'){ $ndays = 'Today';}
										...
										if (!empty($child['PostComment']['comment_image']) && ($child['PostComment']['image_type']=='File')){
											$imagetype = '<img src="'.$child['PostComment']['comment_image'].'"
											 ...
											 $filename = basename($child['PostComment']['comment_image']);  
										   $imagetype = '<img src="'.$child['PostComment']['comment_image'].'"
												 onmouseover=this.src="/img/comments/original/'.$filename.'"
												 onmouseout=this.src="'.$child['PostComment']['comment_image'].'">
											</img>';
									...
									$user = $this->User->find('first', array('conditions' => array('User.id' => $usrid)));
										$username = $user['User']['username'];
                                                                                                                         if (!empty($user['User']['avatar_image'])) {
                                  $img = $user['User']['avatar_image'];
                                 if (filter_var($img, FILTER_VALIDATE_URL) === FALSE) {    $primage = '<a href="/users/profile_post/'.$user['User']['id'].'"><img src="/img/avatar/'.$img.'" style="max-width:10%" /></a>';           
                                 }else{
                                 $primage = '<a href="/users/profile_post/'.$user['User']['id'].'"><img src='.$img.' style="max-width:10%" /></a>';
                                                                                    }   
                                                                                     }else {
										...
										echo '<section class="user-comment-box2">
											...
											<section class="col-lg-9 postrow">
												<span class="userpic">'.$primage.'</span>
												<span class="post-time"><a href="users/profile_post/'.$user['User']['id'].'"><strong>'.$username.'</strong></a></span>
											</section>
	...
	function tag_contents($tagid=NULL){ 
		...
		$this->loadModel('Tag');
		if(!(is_numeric($tagid))){ 
			$tagNameId=$this->Tag->find('first',array('conditions'=>array('Tag.tag_name'=>$tagid),'fields'=>array('Tag.id','Tag.tag_name'))); 
			$tagggName=$tagid; 
			if(!empty($tagNameId)){$tagid = $tagNameId['Tag']['id'];}else{$tagid = '';}
			
		}else{ 
			$tagName=$this->Tag->find('first',array('conditions'=>array('Tag.id'=>$tagid),'fields'=>array('Tag.tag_name'))); 
			$tagggName=$tagName['Tag']['tag_name']; 
		}
		$tagsumcontentids=$this->TagSum->find('all',array('conditions'=>array('TagSum.tag_id'=>$tagid),'fields'=>array('TagSum.content_id'))); 
		$contentIds=array();
		foreach($tagsumcontentids as $tagsumcontentid){$contentIds[]=$tagsumcontentid['TagSum']['content_id'];}
		$contentIds =array_unique($contentIds); 
		$this->Content->bindModel(array('hasMany'=>array('Photo')));
		$contents = $this->Content->find('all', array('conditions' => array('Content.id'=>$contentIds),'order'=>array('Content.id DESC'),'limit' =>9));
		$allcontents = count($contentIds);
		$this->set(compact('contents', 'allcontents','tagggName'));
	}
        
        function tagname_contents($tagname=NULL){ 
		$this->layout = 'index';
		$this->set('title_for_layout', 'Footybase');
		$this->loadModel('Content');
                ...
                if(!empty($tagname)){
                    $this->Content->bindModel(array('hasMany'=>array('Photo')));
                    $contents = $this->Content->find('all', array('conditions' => array("(Content.content_order like '%".$tagname."%' or Content.id in (select content_id from tag_sums where tag_id in (select id from tags where tag_name like '%".$tagname."%'))) and Content.status = 1 and Content.is_deleted=0"), 'order' => 'Content.created desc'));
	...
	function ajaxquizpreviewresult(){
	  ...
	  $preview_results=$this->QuizPreviewResult->find('all',array('conditions'=>array('QuizPreviewResult.quiz_id'=>$quiz_id,'QuizPreviewResult.IP_address'=>$_SERVER['REMOTE_ADDR']),'fields'=>array('QuizPreviewResult.quiz_answer_id','QuizPreviewResult.quiz_question_id'))); 
		$quiz_answer_ids=array();$quiz_questionss_ids=array();
		foreach($preview_results as $preview_result){
			$quiz_answer_ids[]=$preview_result['QuizPreviewResult']['quiz_answer_id'];
			$quiz_questionss_ids[]=$preview_result['QuizPreviewResult']['quiz_question_id'];
		}
		...
		$you_scored= ($total_quiz_answers/$quiz_qstn_count)*100;		
			...
			echo '<section class="col-lg-12">
					<script>
						 ...
						 var qslideCount = $("#quiz_slider ul li").length;
						var qslideWidth = $("#quiz_slider ul li").width();
						var qslideHeight = $("#quiz_slider ul li").height();
						var qsliderUlWidth = qslideCount * qslideWidth;
						$("#quiz_slider ul li:last-child").prependTo("#quiz_slider ul");
							 function quizmoveLeft(){ 
								$("#quiz_slider ul").animate({
									left: + qslideWidth
								}, 200, function () {
									$("#quiz_slider ul li:last-child").prependTo("#quiz_slider ul");
									$("#quiz_slider ul").css("left", "");
								});
							};

							function quizmoveRight() { 
								$("#quiz_slider ul").animate({
									left: - qslideWidth
								}, 200, function () {
									$("#quiz_slider ul li:first-child").appendTo("#quiz_slider ul");
									$("#quiz_slider ul").css("left","");
								});
							};
							$("a.pre-slide").click(function () {
								  
								  quizmoveLeft();
								});

							  $("a.nxt-slide").click(function () {
					...
					<section class="quiz-page" id="quiz_slider">
						<a class="pre-slide" href="javascript:void(0)"><i class="fa fa-angle-left"></i></a>
						<a class="nxt-slide" href="javascript:void(0)"><i class="fa fa-angle-right "></i></a>
						<ul class="pagination page-diff">'; $cntnmb=1;
						   for($i=0;$i < $quiz_qstn_count; $i++,$cntnmb++){ 
						   if($qtn_id==$allqtnIDs[$i] || in_array($allqtnIDs[$i], $quiz_questionss_ids)){
					...
					<section class="quiz-ans quizans-custom quiz-ans-border">';
						...
						echo '<section class="quizans transition" id="quizanswer'.$quiz_answer['id'].'" onclick="savepreviewresult('.$quiz_id.','.$quiz_answer['quiz_questions_id'].','.$quiz_answer['id'].','.$lastqtn.')">
							<div class="media">
	 ...
	 function ajaxquizpreviewanswer(){
	  ...
	  $this->loadModel('QuizQuestion');
		...
		$next_question=$this->QuizQuestion->find('neighbors', array('field' => 'id','value' =>$_POST['quiz_questions_id'],'conditions' =>array('QuizQuestion.quiz_id'=>$_POST['quiz_id'])));
		...
		$next_question_id =$next_question['next']['QuizQuestion']['id'];
		 $return=array($right_answer,$your_answer,$next_question_id); 
        echo json_encode($return);
		//echo $message;
		exit;
	 }
	...
	function ajaxpollpreviewresult(){
      ...
      $this->loadModel('PollPreviewResult');
		...
		$checkvalue=$this->PollPreviewResult->find('first',array('conditions'=>array('PollPreviewResult.poll_id'=>$_POST['poll_id'],'PollPreviewResult.poll_question_id'=>$_POST['poll_questions_id'],'PollPreviewResult.IP_address'=>$_SERVER['REMOTE_ADDR'])));
		if(empty($checkvalue)){
			...
			$totalp=$this->PollPreviewResult->find('count',array('conditions'=>array('PollPreviewResult.poll_id'=>$_POST['poll_id'],'PollPreviewResult.poll_question_id'=>$_POST['poll_questions_id'],'PollPreviewResult.poll_answer_id'=>$_POST['poll_answer_id'])));
			  
			$totalA=$this->PollPreviewResult->find('count',array('conditions'=>array('PollPreviewResult.poll_id'=>$_POST['poll_id'])));
			$pollpercnt=round((($totalp/$totalA)*100),2).'%';
			$totalp = '('.$totalp.' votes)';
			$countpoll = $pollpercnt.' '.$totalp;
    ...
    function fetch_preview($Pid){
		...
		$tagIDs = array_unique($tagsumarray);
		$lastcommentids=$this->PostComment->find('first', array('order' => array('PostComment.id' =>'desc'),'fields'=>array('PostComment.id'))); $lastcommentid = $lastcommentids['PostComment']['id'];
                $this->set(compact('content','users','allpostcomments','allpostcounts','tagIDs','count_page_view','lastcommentid','indexvalue','CCC'));
 }
	function user_login(){
		 $this->layout = 'popup';
        }
...
//          $this->User->setCaptcha($this->Captcha->getVerCode()); //getting from component and passing to model to make proper validation check
            ...
            if ($this->User->validates(array('fieldList' => array('captcha')))) {
                ...
                $userInfo = $this->User->find('first', array('fields' =>
                        array('id', 'first_name', 'last_name', 'email', 'password',
                        'user_type', 'username', 'avatar_image', 'cover_image','bio'),
                        'conditions' => array("User.email" => $email,
                        "User.password" => $user_password,
                        "User.status" => 1, "User.is_deleted" => 0)
                        )
                );
                if (!empty($userInfo['User']['password']) && ($userInfo['User']['password'] == $user_password)) {
                    ...
                    $this->Session->write('UserInfo', $userInfo['User']); 
					 if (!empty($this->request->data['User']['remember_me'])) {
                        ...
                        $password = base64_decode($this->Cookie->read('UserPwd'));
                        ...
                        $this->Cookie->write('UserPwd', base64_encode($cookie_pass), false, '+2 weeks');
                    } else {
                        $email = $this->Cookie->read('UserEmail');
                        $password = base64_decode($this->Cookie->read('UserPwd'));
                        if (!empty($email) && !empty($password)) {
		    ...
		    echo 'Email or Password is incorrect';
                ...
                echo "Please enter the valid Email or Password.";
            }
        }else{
            $email = $this->Cookie->read('UserEmail');
            $password = base64_decode($this->Cookie->read('UserPwd'));
	...
	exit;
	    ...
	    /* Active/Inactive/Delete functionality */
	    if((isset($this->data["Content"]["setStatus"])))
	    {
		    if(!empty($this->request->data['Content']['status'])){
			    $status = $this->request->data['Content']['status'];
		    }else
		    {
			    $this->Session->setFlash("Please select the action.",'default',array('class'=>'alert alert-danger'));	
			    $this->redirect(array('action' => 'content_index'));
		...
		if($value !="") {
            ...
            $this->Paginator->settings = array('conditions' => array($criteria),
				'contain' => array('User' => array('fields' => array('CONCAT(User.first_name, " ", User.last_name) as name'))),
                                'limit' => 10,
				'fields' => array('Content.id','Content.title','Content.status','Content.created'),
				'order' => array(
					'Content.id' => 'DESC'
	    ...
	    $this->set('breadcrumb','Contents');
        }    
         
       
        function admin_list_comments($id=null)
        {
            $id= base64_decode($id);
            $this->set(compact('id'));
            $this->loadModel('PostComment');
	    /* Active/Inactive/Delete functionality */
	    if((isset($this->data["PostComment"]["setStatus"])))
	    {
                if(!empty($this->request->data['PostComment']['status'])){
                    $status = $this->request->data['PostComment']['status'];
                }else{
                    $this->Session->setFlash("Please select the action.",'default',array('class'=>'alert alert-danger'));	
                    $this->redirect(array('action' => 'content_index'));
                }
	    ...
	    $criteria="PostComment.is_deleted =0 and PostComment.content_id='".$id."'"; 
		...
		if($value !="") {
		    $criteria .= " AND (PostComment.comments LIKE '%".$value."%' OR Content.title LIKE '%".$value."%' OR User.first_name LIKE '%".$value."%' OR User.last_name LIKE '%".$value."%')";								
		}
	    }
            $this->PostComment->bindModel(array('belongsTo' => array('User', 'Content')));
            $this->Paginator->settings = array('conditions' => array($criteria),
				'contain' => array('User' => array('fields' => array('CONCAT(User.first_name, " ", User.last_name) as name')), 'Content' => array('fields' => array('title'))),
                                'limit' => 10,
				'order' => array(
					'PostComment.id' => 'DESC'
				)
	    ...
	    $this->set('getData',$this->Paginator->paginate('PostComment'));
	    $this->set('keyword', $value);
        ...
        function admin_delete_comments($content_id = null, $id = null){
            $content_id = base64_decode($content_id);
            
            $id= base64_decode($id);
            $this->loadModel('PostComment');
            $this->PostComment->delete($id);
            $this->Session->setFlash("Comment has been deleted sucessfully.",'default',array('class'=>'alert alert-success'));
            $this->redirect(array('controller' => 'contents', 'action' => 'list_comments', base64_encode($content_id), 'admin' => true));
