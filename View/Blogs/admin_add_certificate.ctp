...
var pricing_id = '';
    ...
    //Show the selected Recipient name in span tag
    $(document).find($("select#GiftCertificateRecipientId")
            .change(function() {
                var stri = "";
                $("select#GiftCertificateRecipientId option:selected").each(function() {
                    if ($(this).text() != "---Please select---") {
                        stri += $(this).text() + " ";
                    }
                });
                $("#customerNameToemail").text(stri);
            })
            .change());
        ...
        if (selectedService) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var chk_duration = $(document).find('.single-check').children('.getduration').attr('id');
                    var chk_pricing = $(document).find('.single-check').children('.pricing-option').attr('id');
                    if(typeof chk_duration != 'undefined'){
                        setValues(chk_duration ,chk_pricing); 
                    }else{
                        duration = $("input[type='radio'][name='data[GiftCertificate][service_amount]']:checked").next().find('.getduration').attr('id');
                        pricing_id = $("input[type='radio'][name='data[GiftCertificate][service_amount]']:checked").next().find('.pricing-option').attr('id');
    ...
    function setValues(duration , pricing){
        $(document).find('.set_duration').val(duration);
        $(document).find('.set_pricing').val(pricing);
    } 
</script>
        ...
        <div class="modal-body clearfix">	
            <div class="box">
                <div class="box-content">
                    ...
                    echo $this->Form->hidden('duration', array('class'=>'set_duration','label' => false, 'div' => false));
                    echo $this->Form->hidden('pricing_option', array('class'=>'set_pricing','label' => false, 'div' => false));
                    ?>
                    <div class="col-sm-6">
                        ...
                        <div class="form-group flatAmountDiv">
                            <div class="col-sm-12">
                               ...
                               <?php echo $this->Form->input('GiftCertificate.amount', array('type'=>'text','class' => 'required form-control number', 'label' => false, 'required', 'validationMessage'=>'Amount is required.','maxlength'=>5)); ?>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-12">
                    ...
                    <div class="col-sm-6">
                        <div class="gift-gallery-box">
                            <div class="form-group">
                                ...
                                <div class="col-sm-7">
                                    <?php
                                    $defaultPrimaryKey = $this->Common->giftImagePrimaryId();
                                    $designCategorylist = $this->Common->imageDesignCategoyList();
...
$(document).ready(function() {
    ...
    $('#GiftCertificateAdminAddCertificateForm .dynamicImageAdmin').load(getGiftImageURL + '/' + 1, function() {
    });
