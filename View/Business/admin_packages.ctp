		...
		<div role="tabpanel" class="tab-pane active" id="Packages">
		    ...
		    <div class="col-sm-3 col-xs-5 nopadding">
                        <select class="package_filter form-control">
                          <option value="2">All Packages</option>
                          <option value="1"> Active </option>
                          <option value="0"> In Active </option>
                        </select>
			</div>
                        <a class="rt-text addPackage" data-id="" title="Add package"><i class="fa fa-plus"></i> Add Package</a>
                    </div>
...
function callfordatarepace(){
        ...
        var imageList = "<?php echo $this->Html->url(array('controller'=>'Business','action'=>'packages','admin'=>true))?>"
 ...
 function deletePricingOption($smallmodal,val){
	...
	if(confirm("Are you sure you want to delete this pricing option?")){
     ...
     function deleteCommon($modal,val,table,deleteType){
        var theId = val;
        if(theId){
            $.ajax({
                url: "<?php echo $this->Html->url(array('controller'=>'SalonServices','action'=>'delete','admin'=>true))?>"+"/"+table,
                type:'POST',
   ...
   $(document).ready(function(){
	...
	/***********Packages********/
       ...
       $(document).on('click','.addPackage',function(){
		...
		if(!id){ id = 0; }
		var theservice = '<?php echo $this->Html->url(array('controller'=>'SalonServices','action'=>'addedit_package','admin'=>true));?>'+'/'+id+'/Package';
	...
	/*****Delete Package******/
		...
		if(confirm('Are you sure you want to delete this package ?')){
		    deleteCommon(modal,value,'Package','temp');
		}
	    });
		...
		var modal = "$(document)";
		if(confirm('Are you sure you want to delete this package?')){
		    deleteCommon(modal,value,'PackageService','permanent');
		}
	    });
	...
	$packagemodal.on('click','.create_pkgdeal',function(){
		...
		var pkgdeal = '<?php echo $this->Html->url(array('controller'=>'SalonServices','action'=>'create_servicedeal','admin'=>true));?>'+'/'+pkgId+'/Package/0';
