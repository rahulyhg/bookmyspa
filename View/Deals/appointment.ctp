...
<?php
	...
	//pr($appointmentDetail); die;
        $amount = $appointmentDetail['price'];
	...
	<div class="deal-banner-rgt">
            ...
            //pr($this->request->data);
			    ...
			    echo __('AED',true);
			     ...
			     $amount = $amount*$appointmentDetail['selected_quantity'];
			      if($tax > 0){
	   ...
	   ?>
		...
		<?php  if(!empty($DealData['Deal'][$lang.'_name'])){
			 echo $DealData['Deal'][$lang.'_name'];
		       }else{
			   echo $DealData['Deal']['eng_name'];
		       } ?>
	    </div>
		...
		<?php
		//pr($packageOwner);
		if(!empty($dealOwner['Salon'][$lang.'_name'])){ echo $dealOwner['Salon'][$lang.'_name']; }else{ echo $dealOwner['Salon']['eng_name']; }?></h5>		
	    </div>
              
              
	    <?php if($appointmentDetail['theBuktype']== "appointment" && ($appointmentDetail['thetype']== "Package" || $appointmentDetail['thetype']== "Spaday")){ ?>
            <div class="duration ">
                  <h5>Date</h5>
            ...
            <?php if($appointmentDetail['theBuktype']== "appointment" && ($appointmentDetail['thetype']== "Service")){ ?>
            <div class="duration ">
                  <h5>Date</h5>
                  <div class="time"><i class="fa fa-calendar"></i> <?php echo $appointmentDetail['selected_date']; ?></div>
            </div>
            <div class="clearfix"></div>
            <?php } ?>
            
            
            <?php if($appointmentDetail['theBuktype']== "appointment" && ($appointmentDetail['thetype']== "Package" || $appointmentDetail['thetype']== "Spaday" || $appointmentDetail['thetype']== "Service")){ ?>
	    ...
	    <?php if($appointmentDetail['theBuktype']!= "appointment"){ ?>
	     <div class="duration" >
		     ...
		     <div class="time"><i class="fa  fa-exclamation-triangle"></i> <?php echo $appointmentDetail['selected_quantity']; ?></div>
	     </div>
	    <?php } ?>
	    ...
	    <!-- <div class="duration bod-non">
		   <h5><?php //echo __('cancel_policy',true); ?></h5>
		   <div class="time"><i class="fa  fa-exclamation-triangle"></i> <?php echo (!empty($ownerPolicy['PolicyDetail']['cancel_appointmenttxt']))?$ownerPolicy['PolicyDetail']['cancel_appointmenttxt'] : ' ---- ';?></div>
	     </div>-->
      </div>
        </div>
        <div class="big-lft appBuklft">
		    ...
		    <?php
		      ...
		      echo $this->Form->create('Appointment',array('url'=>array('controller'=>'Deals','action'=>'payment',$DealData['Deal']['id'],$uid,'payment'))); 
		    ?>
		    <?php echo $this->Form->input('package_id',array('label'=>false,'div'=>false,'class'=>'form-control','type'=>'hidden','value'=>$DealData['Deal']['id'])); ?>
			...
			<?php echo $this->Form->input('order_id',array('value'=>rand(),'label'=>false,'div'=>false,'class'=>'form-control','type'=>'hidden')); ?>
                        ...
                        <?php
			echo $this->Form->input('redirect_url',array('label'=>false,'div'=>false,'class'=>'form-control','style'=>'display:none;','value'=>Configure::read('BASE_URL').'/Deals/payment/'.$DealData['Deal']['id'].'/'.$uid)); ?>
                        <?php echo $this->Form->input('cancel_url',array('label'=>false,'div'=>false,'class'=>'form-control','style'=>'display:none;','value'=>Configure::read('BASE_URL').'/Deals/payment/'.$DealData['Deal']['id'].'/'.$uid)); ?>
				   ...
				   <div class="col-sm-4 no-pdng">
				       <?php echo $this->Form->button('Use Code',array('type'=>'button','label'=>false,'div'=>false,'class'=>'purple-btn promoCode pull-right','readonly'=>false)); ?>
                        ...
                        <div class="col-sm-12 pdng-rgt0">
                           <div class="form-group">
                                <a style="display: none" class="cancel_gift gray-btn pull-left" href='javascript:void(0)'>Cancel</a>
...
<!-- Hidden varibales --------------------->
    <?php
      ...
      echo $this->form->input('form_action' ,array('type'=>'hidden','value'=>'/Deals/payment/'.$DealData['Deal']['id'].'/'.$uid.'/payment'));
