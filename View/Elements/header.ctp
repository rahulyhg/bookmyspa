...
<meta name="google-signin-client_id" content="<?php echo Configure::read('Google.clientId'); ?>">
...
function gift_check(){
	 ...
	 triger_from = $(document).find('.main_login').text();
	  ...
	  if(param=="GiftCertificates" || salonGift=="salon_gift"){
		...
		return true;
	       ...
	       $(document).find('#mySmallModal').modal('toggle');
	      return true;
		  ...
		  redirectTo = $(document).find('a.action').data('login_val');
		  redirectReview = $(document).find('.submitComment').attr('data-uid');
		  console.log(redirectReview);
		  //console.log(redirectTo);
		  
		if (redirectReview=='reviewcomments') {
		    ...
		    if (commentText) {
		     ...
		     window.location.href ='<?php echo $this->Html->url(array("controller"=>"Place","action"=>"index")); ?>';
  ...
  function Google_signIn(googleUser) {
        var profile = googleUser.getBasicProfile();
            if(profile){
                    $.ajax({url: '<?php echo $this->Html->url(array("controller"=>"users","action"=>"googleAuth"))?>', 
                      data:{User:profile},
                      type:'POST',
                      success: function(result){
                      //console.log(result);
  ...
  var startApp = function() {
      ...
      auth2 = gapi.auth2.init({
        client_id: '<?php echo Configure::read('Google.clientId'); ?>',
