    ...
    <div class="panel-heading" role="tab" id="heading-<?php echo $service['SalonService']['id']?>">
      <h4 class="panel-title">
        ...
        <span class="inner-diff-links">
            ...
            <a href="javascript:void(0);" data-parent-id="<?php echo $service['SalonService']['service_id']?>" data-id="null" data-cat = "<?php echo $service['SalonService']['id']?>" class="add addServiceS" title="Add Services"><i class="fa fa-plus"></i></a>
        </span>
      </h4>
    </div>
           ...
           if(!empty($service['children'])){
                 foreach($service['children'] as $treatment){
                               $add_class ='';
                                ...
                                if(count($deal)){
                                         if(in_array($treatment['SalonService']['id'] ,$deal['salon_service_id'])){
                                           foreach($deals as $dealData){
                                                if($dealData['DealServicePackage']['salon_service_id'] ==$treatment['SalonService']['id']){
                    ...
                    <div class="v-deal" data-id="<?php echo $treatment['SalonService']['id']; ?>">
                        <div class="v-deal-box">
                           ...
                           // if(isset($treatment['SalonServiceImage']) && (!empty($treatment['SalonServiceImage']))){
                                 ...
                                 echo $this->Html->image($this->Common->getsalonserviceImage($treatment['SalonService']['id'],$auth_user['User']['id'],350),array('class'=>" ")); 
                               // }else{?>
                            ...
                            <div class="bottom <?php echo $add_class; if(!$treatment['SalonService']['status']){ echo 'dull'; }  ?>">
