...
if(!isset($service_id)){
    $service_id = null;
} $SalonPackages = $this->requestAction(array('controller' => 'search', 'action' => 'getSalonPackages',$user_id, $type, $ser_cat_type, $service_type,$service_id));
$SalonPackages = $SalonPackages['packages'];
?>
  <?php $lang =  Configure::read('Config.language'); ?>     
<h2 class="share-head">Package
    ...
    if(!empty($SalonPackages)){
        ...
        foreach($SalonPackages as $package){
            ...
            <div class="deal-box col-sm-3 col-xs-12 mrgn0 <?php echo $append_class;?>">
                ...
                <?php } ?>
                <section class="sec">
                    <span> <?php echo substr($package['Package'][$lang.'_name'],0,23); ?></span> 
                    ...
                    <?php $service_name = $package['Package'][$lang.'_name'];
                    ...
                    echo $this->Html->link('<button class="book-now" type="button">'.$btn_txt.'</button>',
                            $link_book,
