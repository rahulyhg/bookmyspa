...
class CaptchaHelper extends AppHelper {
  ...
  function render($settings=array()) {
    switch($settings['captchaType']):
      case 'image':
        echo $this->Html->image($this->Html->url(array('controller'=>'admins', 'action'=>'captcha'), true),array('id'=>'img-captcha','vspace'=>2));
        echo '<p><a href="#" id="a-reload">Can\'t read? Reload</a></p>';
        echo '<p>Enter security code shown above:</p>';
        echo $this->Form->input($settings['modelName'].'.'.$settings['fieldName'], array('autocomplete'=>'off','label'=>false,'class'=>'','value'=>''));
...
<?php
  ...
  function render2($settings=array()){
    $settings['modelName'] = 'User';
    switch($settings['captchaType']):
      case 'image':
